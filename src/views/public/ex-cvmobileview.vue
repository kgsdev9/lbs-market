  ...
  <div class="mob-root">
    ...
    <div class="mob-topbar">
      <div class="mob-topbar-left">
        ...
        <div>
          <div class="mob-title">Mon CV Nouveau</div>
...
export default {
  ...
  try {
    ...
    const pages = Math.ceil(totalHeightMm / pageH)
    for (let i = 0; i < pages; i++) {
      if (i > 0) pdf.addPage()
      const sliceY = Math.round((i * pageH) / pxToMm)
      const sliceHeight = Math.round(pageH / pxToMm)
      const ratio = canvas.width / 794
      const pageCanvas = document.createElement('canvas')
      pageCanvas.width = canvas.width
      pageCanvas.height = Math.round(sliceHeight * ratio)
      const ctx = pageCanvas.getContext('2d')
      ctx.drawImage(
        canvas,
        0, Math.round(sliceY * ratio),
    ...
    // Attendre que ReactNativeWebView soit disponible (jusqu'à 3s)
    ...
    if (rnWebView) {
      // WebView mobile → postMessage vers React Native
    ...
    // Envoyer l'erreur à React Native aussi
    if (window.ReactNativeWebView) {
